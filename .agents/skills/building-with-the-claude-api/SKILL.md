...
- Debugging Codex API issues: message shape, tool loops, streaming events, cache misses, stop_reason handling, citation spans, RAG relevance, MCP handshakes.
- Designing a new Codex integration (tool use, RAG pipeline, MCP server/client, Agent Skills, eval harness).

## When NOT to use

- Codex CLI usage, hooks, slash commands, or IDE extension questions — separate skill.
...
response = client.messages.create(
    model="Codex-opus-4-7",  # or Codex-sonnet-4-6, Codex-haiku-4-5
    max_tokens=1024,
...
|---|---|---|
| `Codex-opus-4-7` | Hard reasoning, agentic coding | **Adaptive thinking required** (manual `thinking.type="enabled"` returns 400); start effort at `xhigh` |
| `Codex-sonnet-4-6` | Balanced quality/cost; most apps | Default `effort="high"` — set explicitly to avoid surprise latency |
| `Codex-haiku-4-5` | Cheap/fast: classification, graders, bulk | 200k context; drops on low-resource languages |
| `Codex-opus-4-6` | Only path to Fast mode | 6× price when `speed="fast"` |
...
- **Batch parallel tool results into one user message.** Splitting across turns teaches Codex to stop parallelizing.
