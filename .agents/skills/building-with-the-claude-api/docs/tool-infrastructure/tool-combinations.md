...
Each snippet shows only the `tools` array. See [Handle tool calls](../tools/handle-tool-calls.md) for the full request shape.
...
Pair this with a constrained working directory and a command allowlist if the agent operates on untrusted code. See [Text editor tool](../tools/text-editor-tool.md) and [Bash tool](../tools/bash-tool.md) for the execution contracts.
...
Memory is orthogonal to the rest of your toolset. It doesn't change how other tools behave; it gives Claude a place to write down and later retrieve facts that would otherwise be lost when the context window resets. See [Memory tool](../tools/memory-tool.md) for the storage model.
...
Computer use is the most general option and also the slowest, since every action requires a screenshot roundtrip. Prefer narrower tools when they cover your use case, and reach for computer use when nothing else fits. See [Computer use tool](../tools/computer-use-tool.md) for the sandbox setup.
