...
> See also: [ADDITIONAL-NOTES.md § D7](../ADDITIONAL-NOTES.md#d7--3-tool5-handle-tool-callsmd) — helper-function refactor for multi-block messages (`add_user_message`, `add_assistant_message`, `text_from_message`) and why you must append the full `response.content` list, not just the text.

---

This page covers the tool-call lifecycle: reading `tool_use` blocks from Claude's response, formatting `tool_result` blocks in your reply, and signaling errors. For the SDK abstraction that handles this automatically, see [Tool Runner](tool-runner-sdk.md).

<Note>
**Simpler with Tool Runner**: The manual tool handling described on this page is automatically managed by [Tool Runner](tool-runner-sdk.md). Use this page when you need custom control over tool execution.
</Note>
...
- For running multiple tools in one turn, see [Parallel tool use](parallel-tool-use.md).
- For the SDK abstraction that automates this loop, see [Tool Runner](tool-runner-sdk.md).
- For the full tool-use workflow, see [Define tools](define-tools.md).