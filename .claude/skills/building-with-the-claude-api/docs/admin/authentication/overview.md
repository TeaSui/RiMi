...
- **Send the key:** Set the `x-api-key` header on direct HTTP requests, or set the `ANTHROPIC_API_KEY` environment variable and the [client SDKs](https://docs.claude.com/en/docs/api/client-sdks) pick it up automatically.
...
<CardGroup cols={2}>
  <Card title="Set up Workload Identity Federation" icon="lock" href="/docs/en/build-with-claude/workload-identity-federation">
    Configure issuers, rules, and service accounts, then exchange tokens
  </Card>
  <Card title="Identity provider guides" icon="cloud" href="/docs/en/build-with-claude/workload-identity-federation#identity-providers">
    Step-by-step guides for AWS, Google Cloud, Azure, GitHub Actions, Kubernetes, and Okta
  </Card>
  <Card title="WIF reference" icon="book" href="/docs/en/api/authentication/wif-reference">
